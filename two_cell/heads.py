...
class GNNGraphHead2Cell(nn.Module):
    def __init__(self, dim_in, dim_out, cfg):
        super().__init__()
        self.layer_post_mp = MLP(
            new_layer_config(dim_in * 2, dim_out, cfg.gnn.layers_post_mp, has_act=False, has_bias=True, cfg=cfg)
        )
        self.pooling_fun = global_mean_pool
    ...
    def forward(self, batch):
        batch1, batch2 = batch
        graph1_emb, graph2_emb = self.pooling_fun(batch1.x, batch1.batch), self.pooling_fun(batch2.x, batch2.batch)
